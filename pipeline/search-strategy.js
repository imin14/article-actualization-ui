...
import { truncateToTokenBudget } from './tokenize.js';
...
export function buildClassificationPrompt({ keyword, contextDescription, hits }) {
  ...
  // Hard cap per hit paragraph so a pathologically long paragraph can't blow
  // the 500-token classification budget. 200 tokens ≈ 800 latin chars,
  // plenty for a yes/no decision.
  const PER_HIT_BUDGET = 200;
  const lines = [];
  ...
  for (let i = 0; i < hits.length; i++) {
    const h = hits[i];
    const para = truncateToTokenBudget(h.hit_paragraph, PER_HIT_BUDGET) || h.hit_paragraph.slice(0, 200);
    lines.push(`[${i + 1}] field=${h.field} ¶${h.hit_index}: ${para}`);
  }
  lines.push('');
  lines.push(
