...
function createMockClient() {
  ...
  return {
    async listCampaigns() {
      // Mock: surface the single fixture campaign so the picker has something to show.
      return [{
        id: state.campaign.id,
        topic: state.campaign.topic,
        source_locale: state.campaign.source_locale,
        started_at: state.campaign.started_at || null,
        total: state.blocks.length,
        by_status: state.blocks.reduce((acc, b) => { acc[b.status] = (acc[b.status] || 0) + 1; return acc; }, {}),
        last_updated_at: state.blocks.reduce((acc, b) => (b.updated_at && b.updated_at > acc) ? b.updated_at : acc, ''),
      }];
    },
