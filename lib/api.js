...
function createRealClient({ baseURL, getToken }, overrides = {}) {
  ...
  /** Read endpoints share a transient-failure mode: n8n DataTable's get
   *  operation throws "synchronization error: result count changed during
   *  pagination" when the search workflow inserts rows mid-read. The error
   *  resolves on a fresh read a few hundred ms later. We wrap state reads
   *  with a single auto-retry so the toast doesn't fire on every poll while
   *  search is active. */
  async function fetchStateWithRetry(url, label) {
    const attempt = async () => {
      let r;
      try {
      ...
      if (!r.ok) {
        ...
        const transient = r.status >= 500 || (text && /synchronization error|result count changed/i.test(text));
    ...
    try {
      ...
      // One retry after a short backoff. State polling fires every 8s anyway,
      // so we'd recover on the next tick — but immediate retry hides the
      // glitch from the user.
  ...
  return {
    async listCampaigns() {
      ...
      const data = await fetchStateWithRetry(url, 'listCampaigns');
      if (!data || !Array.isArray(data.campaigns)) throw new Error('Invalid campaigns response');
      return data.campaigns;
    },
    async getCampaignState(campaignId) {
      // campaign-state-fast filters at the DataTable level (campaign_id eq X)
      // instead of pulling every row and JS-filtering. Avoids the n8n
      // pagination race ("synchronization error: result count changed
      // during pagination") that fires constantly while the search workflow
      // is actively inserting rows. Same response shape as the legacy
      // /webhook/campaign-state endpoint.
      const url = `${baseURL}/webhook/campaign-state-fast?campaign_id=${encodeURIComponent(campaignId)}`;
      const data = await fetchStateWithRetry(url, 'getCampaignState');
    ...
    async getCampaignConfig(campaignId) {
      const url = `${baseURL}/webhook/campaign-config?campaign_id=${encodeURIComponent(campaignId)}`;
      let r;
      try {
        r = await fetchWithTimeout(url, { method: 'GET', headers: buildHeaders() }, timeoutMs);
      } catch (e) {
        if (e && e.name === 'AbortError') throw new Error(`Request timed out after ${timeoutMs}ms`);
        throw e;
      }
      if (!r.ok) throw new Error(`getCampaignConfig ${r.status}`);
      return r.json();
    },

    async checkSearchLock(campaignId) {
      const url = `${baseURL}/webhook/search-lock-check`;
      let r;
      try {
        r = await fetchWithTimeout(url, {
          method: 'POST',
      ...
      // 409 is meaningful: caller decides whether to continue or abort.
      if (r.status === 409) {
        const err = new Error(data.reason || 'search-in-progress');
        err.status = 409;
        err.payload = data;
        throw err;
      }
      if (!r.ok) throw new Error(data.error || `checkSearchLock ${r.status}`);
      return data;
    },

    async deleteCampaign(campaignId) {
      const url = `${baseURL}/webhook/campaign-delete`;
      let r;
      try {
        r = await fetchWithTimeout(url, {
          method: 'POST',
      ...
      try { data = JSON.parse(text); } catch { data = { error: text || `HTTP ${r.status}` }; }
      if (!r.ok) {
        const err = new Error(data.error || `deleteCampaign ${r.status}`);
        err.status = r.status;
        throw err;
      }
      return data;
    ...
    async getStoryContent(storyId) {
      const url = `${baseURL}/webhook/story-content?story_id=${encodeURIComponent(storyId)}`;
      let r;
      try {
        r = await fetchWithTimeout(url, { method: 'GET', headers: buildHeaders() }, timeoutMs);
      } catch (e) {
      ...
      if (!r.ok) throw new Error(`getStoryContent ${r.status}`);
      return r.json();
    },

    async saveCampaignConfig(campaignId, config) {
      const url = `${baseURL}/webhook/campaign-config`;
      let r;
      try {
        r = await fetchWithTimeout(url, {
          method: 'POST',
          headers: buildHeaders(),
          body: JSON.stringify({ campaign_id: campaignId, config }),
        }, timeoutMs);
      } catch (e) {
        if (e && e.name === 'AbortError') throw new Error(`Request timed out after ${timeoutMs}ms`);
        throw e;
      }
      if (!r.ok) throw new Error(`saveCampaignConfig ${r.status}`);
      return r.json();
    },
  };
}
